...
def replace_separators(
    value: Any, separators: Sequence[str], new_separator: str = ", "
) -> Any:
  ...
  Args:
    ...
    separators (Sequence[str]): A collection of separators to be replaced.
...
def row_to_custom_str(row: Any, ignore_empty: bool) -> str:
  ...
  if ignore_empty:
    custom_str = [
        f"{col}: {value}" for col, value in row.items() if
        not (pd.isna(value) or (isinstance(value, str) and not value.strip()))
    ]
  else:
